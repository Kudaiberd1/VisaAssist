...
const Card: React.FC<{ trip: Trip }> = ({ trip }) => {
    ...
    const status = trip.progress==100 ? "Завершена" : (trip.progress>0 ? "В процессе" : "Не начата");

    const isFinished = status === "Завершена";

    const statusBgColor = isFinished ? "bg-gray-100" : (status === "В процессе" ? "bg-blue-100" : "bg-yellow-100");
    const statusTextColor = isFinished ? "text-gray-500" : (status === "В процессе" ? "text-blue-800" : "text-yellow-800");
    ...
    return (
        <div className="cursor-pointer rounded-2xl bg-white p-3 sm:p-6 shadow-sm transition-shadow hover:shadow-md">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:gap-6">
                <div className="w-full sm:w-[260px] max-h-[160px] sm:max-h-none mb-8 sm:mb-0">
                ...
                <div className="flex-1">
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        <div>
                            ...
                            <h3 className="text-lg sm:text-xl font-bold text-gray-800">{trip.country}</h3>
                        </div>
                        <span className={`rounded-full px-2.5 py-0.5 text-xs font-semibold ${statusBgColor} ${statusTextColor}`}>
                            {status}
                        </span>
                    </div>
                    <div className="mt-3 flex items-center gap-2 text-sm text-gray-600">
                        ...
                        <span>{trip.startDate ?? trip.tripData?.startDate} — {trip.endDate ?? trip.tripData?.endDate}</span>
                    </div>
                    <div className="mt-4">
                    ...
                    <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <button className={isFinished
                            ? "w-full sm:w-auto rounded-xl bg-gray-100 px-5 py-3 sm:py-2 text-sm font-semibold text-gray-700 hover:bg-gray-200"
                            : "w-full sm:w-auto rounded-xl bg-blue-600 px-5 py-3 sm:py-2 text-sm font-semibold text-white hover:bg-blue-700"
                        } onClick={ () => { nav(`/dashboard/${trip.id}`) } }>
